...
"""
    ...
    ,each:"""
{precalculate}
var refer = {expression0};
refer = ToHDF(refer)
for(var index=0;index<refer.nodes.length;index++){
    HdfRef('{name}',refer.nodes[index]);
    {body}
    ...
    for action in flattendExpression
        ...
        if action.type is "init"
            ...
            if action.valueType is "string"
                results.push ["var ",tVarName,"=","CSValue(\'",action.value,"\','string');"].join("")
            else if action.valueType is "number"
                results.push ["var ",tVarName,"=","CSValue(",action.value,",'number');"].join("")
            else if action.valueType is "hdfQuery"
                results.push ["var ",tVarName,"=","CSValue('",action.value,"','hdfQuery');"].join("")
            else
            ...
            results.push ["var ",context.getVariableName(action.id),"=","CSOperate_"+operateName+"(",params,");"].join("")
...
translate = (node,context)->
    ...
    if typeof node.validString is "string"
        ...
        validString = null
    code = fillTemplate(template,{body:bodyCode,expression:expressions,precalculate:precalculateCode,name:name,hdf:hdf,validString:validString,elseBody:elseBodyCode,elseIfs:elseIfCodes})
